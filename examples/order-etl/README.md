...
筛选已付款订单，计算数量乘以单价的金额，并按 40000 分阈值分为 priority / standard。取消订单会从当前履约关系撤回。
...
初始目标结果：订单 5001，金额 50000 分，priority；待付款的 5002 不在目标中。
...
```sql
SELECT order_id, amount, convert_from(lane, 'UTF8') AS lane
FROM ops.fulfillment ORDER BY order_id;
```
...
```sh
# 5002 付款并改量，目标出现 45000 / priority。
podman compose exec -T postgres psql -U dogpaddle -d dogpaddle -v ON_ERROR_STOP=1 \
  < steps/01-pay-and-reprice.sql

# 新增 5003，目标出现 15000 / standard。
podman compose exec -T postgres psql -U dogpaddle -d dogpaddle -v ON_ERROR_STOP=1 \
  < steps/02-create-paid-order.sql

# 5001 数量变为 20，金额变为 100000。
podman compose exec -T postgres psql -U dogpaddle -d dogpaddle -v ON_ERROR_STOP=1 \
  < steps/03-increase-quantity.sql
...
端口冲突时编辑 `.env` 的 `DOGPADDLE_ORDER_ETL_PG_PORT`，重新加载后再启动。同一 shell 可切换不同 example，它们使用场景独立变量。
