...
已付款且折后金额达到 10000 分的订单进入履约队列。SQL 计算折扣、按 40000 分划分处理通道，并通过 UNION ALL 将国内与其他地区订单路由到不同履约中心。
...
初始目标结果：只有订单 103，金额 24000 分，CN-HUB / standard。101 尚未付款，102 金额低于门槛。
...
```sql
SELECT order_id, payable_cents,
       convert_from(fulfillment_center, 'UTF8') AS center,
...
```sh
# 101 付款，出现 13500 / CN-HUB / standard。
podman compose exec -T postgres psql -U dogpaddle -d dogpaddle -v ON_ERROR_STOP=1 \
  < steps/01-pay.sql

# 103 改量并取消折扣，变为 48000 / CN-HUB / priority。
podman compose exec -T postgres psql -U dogpaddle -d dogpaddle -v ON_ERROR_STOP=1 \
  < steps/02-reprice.sql
...
步骤可选，不维护进度。预期值只适用于初始数据及上述顺序，手动修改后以实际数据为准。更完整的崩溃恢复与连续变更验收使用同一份 SQL，见 [演示说明](../../docs/demo/)。
...
端口冲突时编辑 `.env` 的 `DOGPADDLE_FULFILLMENT_PG_PORT`，重新加载后再启动。同一 shell 可切换不同 example，它们使用场景独立变量。
