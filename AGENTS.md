...
公开的 `Flow::advance` 必须真实按派生的确定性拓扑 schedule 为每个 Station 至多提供一个 turn。`Idle` 丢弃本次写事务并保留输入；`Commit(Keep)` 在同一写事务中提交 Operation continuation 和可选 output，但不得推进 cursor、轮转 active input 或清空 cache；`Commit(Complete)` 才在同一事务中提交 Operation 状态、可选 output、cache 所属 input 的 cursor 推进和 active input 轮转，并且只有 commit 成功后才清空 cache。Operation 即使 output 已达容量也仍可运行；无 output 的 Commit 可以提交，实际 output 则由 AppendLog 的 capacity-aware append 判定。容量拒绝不是错误，必须回滚本次 Operation 事务并保留 durable claim 和原 cache，Source position、`Keep` continuation、`Complete` cursor/active 都不得提前前进。只要尚未 `Complete`，下一 turn（包括 reopen 后）必须提供同一 `(port, offset, bytes)` 标识的完整 Change；cache 还必须在每次处理前同时校验 durable active input 和 cursor。每个成功 Station turn 后 Flow 对其每个不同的直接上游触发一次有界 GC，不得因某个 Station 背压而跳过后续 schedule。只有 committed physical head 前进才释放 retained bytes 并算 GC progress；consumer cursor 只给出安全 target。公共结果按 `Progressed > Backpressured > Idle` 聚合，其中 durable pin 和物理 GC 也属于 Progressed。当前仍不公开 `Flow::start`。
