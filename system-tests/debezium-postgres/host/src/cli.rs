...
mod tests {
    ...
    #[test]
    fn parses_only_the_word_command_protocol() {
        assert_eq!(
            Command::parse("poll 250").unwrap(),
            Command::Poll { timeout_ms: 250 }
        );
        assert_eq!(Command::parse("save").unwrap(), Command::Save);
        assert_eq!(Command::parse("ack 7").unwrap(), Command::Ack { token: 7 });
        assert!(Command::parse(r#"{"command":"poll","timeout_ms":500}"#).is_err());
    }
