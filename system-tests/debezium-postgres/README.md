...
From the repository root, the full local entry point is:

```bash
system-tests/debezium-postgres/scripts/check.sh
```

It builds and checks the independent Rust host with its own lockfile, builds the
product Java distribution in the digest-pinned Maven image, assembles the pinned
Linux x86_64 runtime payload, pulls the pinned PostgreSQL fixture image, then
calls the scenario runner with absolute paths.

To rerun only the scenario against existing artifacts:

```bash
system-tests/debezium-postgres/scripts/run.sh \
  --bundle /absolute/path/to/dogpaddle-debezium-runtime-x86_64-unknown-linux-gnu \
  --host /absolute/path/to/dogpaddle-debezium-d1-host \
  --artifacts-dir /absolute/path/to/new-artifact-directory
```

`run.sh` only executes the already-built fixture. It never builds, downloads, or
audits; the pinned PostgreSQL image must already exist locally. It owns an
exclusive fixture lock, refuses an existing Compose project or scenario state,
and always attempts to capture Compose status, PostgreSQL logs, host logs, and
cleanup output. Without `--artifacts-dir`, its temporary evidence is removed.

Required tools for `check.sh` are the repository-pinned Rust 1.96 with Clippy, Podman with a
working Compose provider, `psql`, Python 3, `curl`, `tar`, `flock`, and a SHA-256
tool. No host JDK, Maven, or `unzip` is required. Connector invocation clears
Java-related environment and loader paths, so the Maven image's Java cannot
replace the payload runtime. Port `55432` must be free.

The `Debezium PostgreSQL recovery` GitHub workflow builds the Linux runtime/D1
host and the three native PostgreSQL hosts in two independent jobs. D1, CDC,
and Sink then consume only artifacts from that workflow and always upload
separate complete log trees. The required summary is
`PostgreSQL engine, scan and sink recovery`.
...
```bash
crates/debezium/scripts/audit-upstream-contract.sh
```
...
`io.debezium.*` and declares no Java-to-Rust native callback. It is a product
pin-review tool, deliberately outside ordinary D1 `check.sh`, `run.sh`, and
routine CI workflows.
