...
The command exits non-zero on the first failed gate. It runs the product
distribution builder inside the pinned Maven/JDK image, runs
...
The upstream source-string audit is intentionally separate at
`crates/debezium/scripts/audit-upstream-contract.sh`; neither the local
`check.sh` nor the artifact-only `run.sh` invokes it.
...
| Java build | Eclipse Temurin 21 / Maven 3.9.11 image; digest pinned by `check.sh` and CI, invoking the unchanged local-only product script |
| Configured runtime | Eclipse Temurin JRE `21.0.12.1+1`, Linux GNU x86_64 archive and upstream SBOM locked by SHA-256 |
| Rust | repository-pinned `1.96.0`; unsafe code forbidden |
...
The pin-upgrade audit also mechanically rejects a product-bridge
`io.debezium.*` source/class shadow and any Java-to-Rust native callback. The
remaining D1 boundaries are independent repository and fixture evidence: the
D1 tree has no Java bridge or Maven manifest, its host has no direct `jni`
dependency or JNI module, and host/configuration tests reject
`connector_and_driver` LSN flushing and caller-controlled Kafka Connect
offset-store properties.
