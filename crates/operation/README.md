...
[`operation::source::SequenceSourceOperation`] 直接持有 `Cell<u64, Small>`，保存最后一次
...
[`operation::transform::CountOperation`] 将直接持有的 `Cell<u64, Small>` 加一并返回新计数；
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let count = store.create_data::<Cell<u64, Small>>("count")?;
...
一个 Operation 的 tag、payload、逻辑数据名称、类型化 collection、K/V codec 和 `SIZE`
