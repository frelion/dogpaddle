...
`GroupState`、`JoinContinuation` 与 `AsOfContinuation` 等状态 codec、collection 的 key/value codec，以及 Flow 加上的
...
`Aggregate` 的 MIN/MAX、`EquiJoin` 的 match/presence transition 和 `AsOfJoin` 的 ordered lookup/
historical rematch 各有 owner benchmark；其他组合
