...
product_bridge_source="$crate_dir/bridge/src"
audit_root="$(mktemp -d "${TMPDIR:-/tmp}/dogpaddle-debezium-upstream-audit.XXXXXX")"
trap 'rm -rf -- "$audit_root"' EXIT

for command in find git mktemp rg rm; do
...
echo "PASS Debezium upstream contract audit $DEBEZIUM_TAG $DEBEZIUM_COMMIT"
