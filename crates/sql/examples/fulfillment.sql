...
INSERT INTO postgres(
    sink_id => 'fulfillment_queue',
    host => '127.0.0.1',
    port => env('DOGPADDLE_FULFILLMENT_PORT'),
    database => 'postgres',
    user => env('DOGPADDLE_FULFILLMENT_USER'),
    password => env('DOGPADDLE_FULFILLMENT_PASSWORD'),
    schema => 'ops',
...
WITH raw_orders AS (
    ...
    FROM postgres_cdc(
        engine_name => 'orders',
        runtime_bundle => env('DOGPADDLE_FULFILLMENT_BUNDLE'),
        host => '127.0.0.1',
        port => env('DOGPADDLE_FULFILLMENT_PORT'),
        database => 'postgres',
        user => env('DOGPADDLE_FULFILLMENT_USER'),
...
qualified AS (
    SELECT
        ...
        payable_cents,
        CASE
            WHEN payable_cents >= 40000 THEN 'priority'
            ELSE 'standard'
        END AS handling_lane
    FROM enriched
...
SELECT
    ...
    subtotal_cents,
    payable_cents,
    'CN-HUB' AS fulfillment_center,
    handling_lane,
    'regional_route' AS handling_reason
FROM qualified
...
SELECT
    ...
    subtotal_cents,
    payable_cents,
    'GLOBAL-HUB' AS fulfillment_center,
    handling_lane,
    CASE
        WHEN payable_cents >= 40000 THEN 'export_review'
        ELSE NULL
    END AS handling_reason
FROM qualified
