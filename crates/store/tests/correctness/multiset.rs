...
use dogpaddle_store::{
    MultisetEntry, OrderedMultiset, PartitionedMultiset, ScanDirection, Store, StoreError,
};
...
fn partitioned_multiset_orders_binary_keys_and_survives_reopen() {
...

    {
        ...
        let mut values = multiset.access(transaction.access()).unwrap();
        {
            ...
            assert_eq!(
                partition.scan(ScanDirection::Ascending, two).unwrap(),
                vec![
                    MultisetEntry {
                        key: keys[0].clone(),
                        multiplicity: 1,
            ...
            assert_eq!(
                partition.scan(ScanDirection::Descending, two).unwrap(),
                vec![
                    MultisetEntry {
                        key: keys[5].clone(),
                        multiplicity: 6,
...
fn empty_partition_has_no_bounds_or_scan_entries_across_reopen() {
...

    {
        ...
        assert!(
            empty
                .scan(ScanDirection::Ascending, NonZeroUsize::MIN)
                .unwrap()
                .is_empty()
        );
    }
